...
TEST(KernelCallableCache, MintedIdsAreSequentialAndCloseInvalidatesThemAll) {
    ...
    KernelCallableResidency found;
    ASSERT_EQ(cache.resolve(3, found), 0);
    // clear() is what close does to the host metadata: every minted id stops
    // resolving at once, which is what replaces a per-callable generation.
    cache.clear();
    for (int id = 0; id < 4; ++id)
        EXPECT_EQ(cache.resolve(id, found), PTO_RUNTIME_ERR_CALLABLE_NOT_RESIDENT);
    // A fresh registration after clear starts the id space over.
    ASSERT_EQ(prepare(cache, device, 0, image()), 0);
    cache.commit(0);
    EXPECT_EQ(cache.resolve(0, found), 0);
}
