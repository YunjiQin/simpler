...
def _run_lifecycle_retry(arch, runtime, device, scenario):
    ...
    try:
        ...
        if scenario == "fatal_device":
            ...
            _check_prepare_registration(lib, ctx, arch, runtime, capture_mode=mode)
            finalized = True
...
def _check_prepare_registration(lib, ctx, arch, runtime, *, close=True, capture_mode=None):
    ...
    capture_stream = ctypes.c_void_p()
    model = ctypes.c_void_p()
    original_thread_mode = ctypes.c_int(capture_mode or 0)
    if capture_mode is not None:
        ...
        assert lib.aclrtCreateStream(ctypes.byref(capture_stream)) == 0
        # The thread override is independent of the capture mode passed to Begin.
        assert lib.aclmdlRICaptureThreadExchangeMode(ctypes.byref(original_thread_mode)) == 0
        assert lib.aclmdlRICaptureBegin(capture_stream, capture_mode) == 0
    faults = ctypes.CDLL(None)
    ...
    assert faults.finish_prepare_guard() == 0
    if capture_mode is not None:
        previous = ctypes.c_int(2)
        assert lib.aclmdlRICaptureThreadExchangeMode(ctypes.byref(previous)) == 0
        assert previous.value == capture_mode
        assert lib.aclmdlRICaptureThreadExchangeMode(ctypes.byref(previous)) == 0
        assert lib.aclmdlRICaptureEnd(capture_stream, ctypes.byref(model)) == 0
        assert lib.aclmdlRIExecuteAsync(model, capture_stream) == 0
        assert lib.aclrtSynchronizeStream(capture_stream) == 0
        assert lib.aclmdlRIDestroy(model) == 0
        assert lib.aclrtDestroyStream(capture_stream) == 0
        assert lib.aclmdlRICaptureThreadExchangeMode(ctypes.byref(original_thread_mode)) == 0
