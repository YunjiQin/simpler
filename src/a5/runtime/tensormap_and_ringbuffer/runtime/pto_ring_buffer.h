...
// PTO2FaninPool definition and fanin iteration helpers (for_each_fanin_*) live
// in pto_fanin_pool.h — they form one cohesive module that PTO2SharedMemoryRingHeader
// embeds without pulling in this header's wider ring-buffer surface.
...
struct PTO2RingSet {
    ...
    // fanin_pool moved to PTO2SharedMemoryRingHeader (sm_header->rings[r].fanin_pool)
    // so sched/wire can access it without dereferencing through the orch struct.
};
