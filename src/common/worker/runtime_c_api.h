...
/**
 * Stage one callable for kernel-mode launches, including inside ACLGraph capture.
 *
 ...
 * asynchronous device work on context-owned streams, without synchronizing
 * any stream or the device. Context-owned H2D uploads use synchronous aclrtMemcpy
 * with the thread capture mode temporarily relaxed and then restored. On success
 * the image is uploaded, but AICPU registration may still be pending; execution
 * errors surface when the caller warms up and synchronizes. Init must complete
 * outside capture. Preparation neither accepts nor
