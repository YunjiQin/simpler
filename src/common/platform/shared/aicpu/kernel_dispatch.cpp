...
extern "C" __attribute__((visibility("default"))) int simpler_aicpu_kernel_exec(void *arg) {
    ...
    // The device entry ABI has no independent packet length. The caller must
    // provide at least the prefix and an allocation matching packet_bytes.
    ...
    return consume_kernel_invocation(
        invocation, *reinterpret_cast<const ChipCallable *>(args.chip_callable_address),
