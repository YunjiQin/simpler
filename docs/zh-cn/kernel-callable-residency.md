...
实现基于 K2 提交 `abbe07f5` 的持久资源机制；prepare 可在 capture 内执行，不同步任何 stream 或整个设备。
...
不进入 callable ID 或注册记录。init 必须在 capture 外完成，prepare 可以在 capture 内调用；同一 context 的
...
simpler_kernel_mode_prepare_callable
  ...
  │   ├─ 校验镜像布局，拒绝越界、非法 signature、名称和子项偏移，以及超出 [0, 1024) 或重复的子核 func_id
  │   ├─ 检查未提交注册、8192 项限制与累计字节预算
  │   ├─ 分配新 ID，保存独立 Host 镜像副本
  │   └─ 在已有块内或新块取得新地址、修补子地址并上传
  ├─ record_callable_on_runner：保存 runtime 注册信息
  ├─ prepare_kernel_callable：enqueue AICPU 注册，准备持久参数，不等待流完成
  ├─ cache.commit：发布 ID（设备注册可能仍在执行）
  └─ 写出 ID，返回 0
```

callable 镜像、Runtime、KernelArgs 和寄存器地址表的上传统一使用 `capture_memcpy_h2d`：
先通过 `aclmdlRICaptureThreadExchangeMode` 临时切到 `RELAXED`，调用同步 `aclrtMemcpy`，
随后恢复原线程模式；拷贝失败也会执行恢复，切换或恢复失败会返回错误。
这些上传立即执行，不录入 graph；设备分配保留到 context close，因此局部 Host 缓冲区可以在返回后释放。
成功返回表示上传完成、注册工作已 enqueue，不保证 AICPU 注册执行完成。
后续 launch 与注册使用同一 AICPU stream 保证顺序，执行错误由调用方 warmup＋synchronize 暴露。
当前 Host launch binder 尚未接通，现有测试覆盖 capture 内 prepare、capture 结束及同步，不等同于完整 kernel 执行验收。

接口依据：[CANN capture mode 切换说明](https://www.hiascend.com/document/detail/zh/canncommercial/850/API/appdevgapi/aclcppdevg_03_1788.html)。
...
TRB 通过 `enqueue_device_register` 在 context 的 AICPU stream 上发射注册任务，加载 orchestration SO。
...
设备侧独立入口为 `simpler_aicpu_kernel_exec`，接收 `SimplerKernelDispatchArgs` 加紧随其后的 payload。
Host binder 从 `cache.resolve(id)` 的驻留结果填写 `chip_callable_address` 和 `chip_callable_bytes`，
指向已上传并修补子 kernel 地址的完整设备 `ChipCallable` 镜像；不传 Host 镜像地址。
入口检查包长、kernel 模式、8192 项 ID 范围、参数数量、保留字段，以及 callable 地址对齐和长度溢出，
再把 `const ChipCallable &`、镜像长度和 payload 传给 `consume_kernel_invocation`。
AICPU consumer 可据 `child_func_ids_` 和各子项 `resolved_addr()` 重建当前 invocation 的 kernel function 表。
consumer 在实际访问子项前仍需确保缓存可见性、校验子项偏移/长度及签名，不能将地址范围检查视为内容校验。
包不携带 generation 或驻留描述符；调用方保证真实分配长度与 `packet_bytes` 一致。
program 的 `simpler_aicpu_exec` 接收 `KernelArgs`，不作为 kernel 协议的 fallback。
当前 production consumer 返回 `UnsupportedPayload`；后续 runtime consumer 需依据设备 callable 重建函数表，并校验
实际签名和 payload 后才能执行。Host binder 尚未接入此设备入口。
